...
class PF_TRON1AFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    max_iterations = 5000         # 较短的训练周期，适合平地环境 / Shorter training for flat terrain
...
@configclass
class hzs_stairs(RslRlOnPolicyRunnerCfg):
    num_steps_per_env =48  #给机器人更多时间展示完整的跨步动作
    max_iterations = 2000         # 增加训练周期，适合楼梯环境 / Shorter training for stair environment
    save_interval = 200           # 更频繁的保存 / More frequent saving
    experiment_name = "hzs_stair"
    empirical_normalization = False
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=1.0,
        actor_hidden_dims=[512, 256, 128],
...
# PF_TRON1A楼梯训练配置 - 针对特定机器人型号的优化配置
@configclass
class hzs_stairs_without_height(RslRlOnPolicyRunnerCfg):
    num_steps_per_env =24  #给机器人更多时间展示完整的跨步动作
    max_iterations = 2000        # 增加训练周期，适合楼梯环境 / Shorter training for stair environment
    save_interval = 200           # 更频繁的保存 / More frequent saving
    experiment_name = "hzs_stair_without_height"
    empirical_normalization = False
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=1.0,
        actor_hidden_dims=[512, 256, 128],
...
@configclass
class hzs_hoppingcfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 3000       
    save_interval = 200           # 更频繁的保存 / More frequent saving
    experiment_name = "hzs_hopping"
    empirical_normalization = False
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=1.0,
        actor_hidden_dims=[512, 256, 128],
