...
class CommandCfg(BaseCommandsCfg):
    ...
    def __post_init__(self):
        ...
        self.base_velocity.rel_standing_envs = 0.0       # 站立环境比例 / Standing environments ratio
...
class ObservarionsCfg:
    ...
    class HistoryObsCfg(ObsGroup):
        ...
        def __post_init__(self):
            ...
            # # 使用的 ActorCritic 网络是 MLP (多层感知机)，它只能处理扁平的向量。
            # # 如果是 False，数据形状是 (N, 10, Dim)，MLP 会报错或处理错误。
            # # 改为 True 后，数据形状变成 (N, 10 * Dim)，网络能一次性看到过去10帧的所有数据。

            # self.flatten_history_dim = True  # 不展平历史维度 / Don't flatten history dimension
...
class RewardsCfg:
    ...
    pen_base_height = RewTerm(
        ...
        params={"target_height": 0.66,},            # 目标高度 66cm / Target height 66cm
        weight=-10.0,                               # 负权重表示惩罚 / Negative weight indicates penalty
    )
