...
############################
# 双足机器人干扰平地环境 / Pointfoot Blind Flat Environment
############################



@configclass
class DisturbFlatEnvCfg(PFBaseEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        ...
        self.events.push_robot = EventTerm(
            ...
            mode="interval",              # 间隔触发模式
            interval_range_s=(0.0, 0.0),  # 每 2-4 秒推一次
            params={
                # 作用在机器人的基座上
...
@configclass
class PFStairsEnvCfg(PFBaseEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.height_scanner = RayCasterCfg(
            prim_path="{ENV_REGEX_NS}/Robot/base_Link",  # 传感器安装在基座上
            attach_yaw_only=True,                         # 仅跟随偏航角旋转，保持水平扫描
            pattern_cfg=patterns.GridPatternCfg(
                resolution=0.05,                          # 扫描网格分辨率 5cm
                size=[0.5, 0.5]                           # 扫描区域大小 0.5m x 0.5m
            ), 
            debug_vis=False,                              # 是否显示调试射线
            mesh_prim_paths=["/World/ground"],            # 射线检测的目标（地面）
        )

        # ---------------------------------------------------------
        # 2. 启用观测项 (Enable Observations)
        # ---------------------------------------------------------
        # 策略网络观测 (带噪声)
        self.observations.policy.heights = ObsTerm(
            func=mdp.height_scan,
            params={"sensor_cfg": SceneEntityCfg("height_scanner")},
            noise=GaussianNoise(mean=0.0, std=0.01),      # 添加高斯噪声模拟真实传感器误差
            clip=(-4, 4),                             # 裁剪数值范围
        )
        ...
        self.observations.obsHistory.heights = ObsTerm(
            func=mdp.height_scan,
            params={"sensor_cfg": SceneEntityCfg("height_scanner")},
            noise=GaussianNoise(mean=0.0, std=0.01), # 历史数据通常也需要带噪声
            clip=(-4, 4), 
        )

        self.observations.obsHistory.flatten_history_dim = True
        ...
        # self.rewards.pen_base_height.weight = -2.0
        ...
        #这里做一下修改，因为爬楼梯的话需要z轴上的速度，所以减小这个惩罚
        self.rewards.pen_lin_vel_z.weight = -0.3          # 减小Z方向速度惩罚 / Decrease Z velocity penalty
        ...
        #有了传感器，机器人应该能看见台阶，所以应该更严格地惩罚“膝盖撞台阶”或“肚子磕台阶”的行为
        self.rewards.pen_undesired_contacts.weight = -2.0
...
@configclass
class PFBlindStairEnvCfg_hzs_without_height(PFBaseEnvCfg):
    """盲视楼梯环境配置 - 专门训练爬楼梯能力 / Blind stairs environment configuration - specialized for stair climbing training"""
    ...
    def __post_init__(self):
        ...
        # 调整奖励权重以适应楼梯爬升 / Adjust reward weights for stair climbing
        #这里很关键，因为base里面把高度是写死的，但是在上楼梯的时候高度是会变化的，所以需要把这个惩罚去掉
        self.rewards.pen_base_height.weight = 0.0

        self.rewards.rew_lin_vel_xy.weight =3.0          
        self.rewards.rew_ang_vel_z.weight = 1.5           # 增加角速度跟踪奖励 / Increase angular velocity tracking reward
         #这里做一下修改，因为爬楼梯的话需要z轴上的速度，所以这个应该是正的
...
@configclass
class PFBlindStairEnvCfg_hzs_without_height_play(PFBaseEnvCfg_PLAY):
    ...
    def __post_init__(self):
        ...
        self.commands.base_velocity.ranges.lin_vel_x = (0.3, 1.0)    # 固定前进速度范围 / Fixed forward velocity range
        ...
        self.scene.terrain.terrain_generator = STAIRS_TERRAINS_PLAY_CFG.replace(difficulty_range=(0.6, 0.6))
...
@configclass
class PFHoppingEnvCfg_HZS(PFBaseEnvCfg):
    """
    兔子跳 (Hopping) 专用配置 - 助教建议版 (From Scratch + Balance Focus)
    """
    def __post_init__(self):
        super().__post_init__()
        ...
        # 腾空比例：接触地面时间 0.3~0.5 (即 50%~70% 时间在腾空)
        self.commands.gait_command.ranges.durations = (0.3, 0.5)
        # 摆动高度：稍微给高一点，方便腾空
        self.commands.gait_command.ranges.swing_height = (0.15, 0.25)
        # 频率：适中，太快容易翻
        self.commands.gait_command.ranges.frequencies = (1.5, 2.5)

        # =========================================================
        # 3. 速度命令 (降低难度)
        # =========================================================
        # 从原地起跳开始练 (0.0)，慢慢加速到 1.5
        self.commands.base_velocity.ranges.lin_vel_x = (1.0, 1.0)
        self.commands.base_velocity.ranges.lin_vel_y = (0.0, 0.0) # 锁死横向
        self.commands.base_velocity.ranges.ang_vel_z = (-0.0, 0.0) # 锁死转向
        ...
        self.rewards.test_gait_reward.weight = 2.0 
        
  
        self.rewards.pen_base_height.weight = 0.0 

        self.rewards.pen_feet_distance.params["max_feet_distance"] = 0.25
